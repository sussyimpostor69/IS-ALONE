...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        ...
        public float MovingTurningSpeedNonCombat = 2000;
        public int StationaryTurningSpeedCombat = 50;
        public int MovingTurningSpeedCombat = 2000;
        ...
        void Update()
        {
            ...
            if (OptimizedStateRef == OptimizedState.Inactive || DisableAIWhenNotInViewRef == YesOrNo.No)
            {
                ...
                if (IsMoving)
                {
                    TurnSpeedTimer = Mathf.LerpAngle(TurnSpeedTimer, 1, Time.deltaTime * 1.5f);
                    TurnSpeedTimer = Mathf.Clamp(TurnSpeedTimer, 0, 1);
                    IsTurning = false;

                    if (TurnSpeedTimer >= 0.9f && CombatStateRef == CombatState.NotActive)
                        m_NavMeshAgent.angularSpeed = MovingTurningSpeedNonCombat;
                    else if (CombatStateRef == CombatState.Active)
                        m_NavMeshAgent.angularSpeed = MovingTurningSpeedCombat;
                }
                else
                {
                    TurnSpeedTimer = 0;
        ...
        void MoveAIRootMotion()
        {
            ...
            Vector3 velocity = Quaternion.Inverse(transform.rotation) * m_NavMeshAgent.desiredVelocity;
            float angle = Mathf.Atan2(velocity.x, velocity.z) * 180.0f / 3.14159f;
            float Dist = Vector3.Distance(m_NavMeshAgent.destination, transform.position);
            ...
            if (AIAgentActive)
            {
                ...
                if (IsAttacking || IsEquipping || IsEmoting || m_NavMeshAgent.remainingDistance <= m_NavMeshAgent.stoppingDistance || IsBlocking || IsTurning)
                {
                    ...
                    m_NavMeshAgent.speed = Mathf.LerpAngle(m_NavMeshAgent.speed, 0f, Time.deltaTime * 30);
                ...
                else if (m_NavMeshAgent.remainingDistance > m_NavMeshAgent.stoppingDistance  && !IsGettingHit && !IsAttacking && !IsSwitchingWeapons && !IsBackingUp && !IsEmoting && !IsEquipping && !IsBlocking && !IsTurning)
                {
                    ...
                    if (DelayTimer >= 0.1f)
                    {
                        ...
                        if (m_NavMeshAgent.remainingDistance < (m_NavMeshAgent.stoppingDistance + 4f) && BehaviorRef != CurrentBehavior.Cautious && ConfidenceRef != ConfidenceType.Coward)
                        {
                            ...
                            {
                                m_NavMeshAgent.speed = Mathf.LerpAngle(m_NavMeshAgent.speed, 0.1f, Time.deltaTime * 30);
                            }
                        }
                        else
                        {
        ...
        void AlignAIMoving()
        {
            if (m_NavMeshAgent.desiredVelocity.magnitude >= 0.05f)
            {
                if (!IsMoving)
                {
